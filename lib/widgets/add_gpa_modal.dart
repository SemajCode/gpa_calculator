...
class AddGpaModal extends StatefulWidget {
  const AddGpaModal({required this.onAddCourse, super.key});

  final void Function(Course course) onAddCourse;

  @override
  State<AddGpaModal> createState() => _AddGpaModalState();
}

class _AddGpaModalState extends State<AddGpaModal> {
  final _courseCodeController = TextEditingController();
  Grade _selectedGrade = Grade.C;
  int _selectedUnit = 3;
  int _calPoint = 0;
    ...
    if (_courseCodeController.text.trim().isEmpty) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const StyleText(text: 'Invalid Input'),
          content:
              const StyleText(text: 'You can\'t submit an empty course code!'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(ctx);
